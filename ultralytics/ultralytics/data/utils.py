    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                if lb.shape[1] >= 6:
                    # class cx cy w h visibility [extra columns ignored]
                    visibility = lb[:, 5:6]
                    lb = lb[:, :5]
                else:
                    # no visibility provided
                    visibility = np.ones((lb.shape[0], 1), dtype=np.float32)
        ...
        return im_file, lb, visibility, shape, segments, keypoints, nm, nf, ne, nc, msg

    except Exception as e:
        nc = 1
        # visibility = np.zeros((0, 1), dtype=np.float32)
        msg = f"{prefix}{im_file}: ignoring corrupt image/label: {e}"
        return None, None, visibility, None, None, None, nm, nf, ne, nc, msg
