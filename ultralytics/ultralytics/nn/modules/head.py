...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: tuple = ()):
        ...
        self.cv_vis = nn.ModuleList(
            nn.Conv2d(x, 1, 1) for x in ch
        )
    ...
    def forward(self, x: list[torch.Tensor]) -> list[torch.Tensor] | tuple:
        ...
        for i in range(self.nl):
            pred = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
            vis = torch.sigmoid(self.cv_vis[i](x[i]))
            x[i] = torch.cat((pred, vis), 1)

            if not self.training:
                print(f"[DEBUG] Head output shape (scale {i}):", x[i].shape)
    ...
    def _inference(self, x: list[torch.Tensor]) -> torch.Tensor:
        ...
        # box, cls, _ = x_cat.split((self.reg_max * 4, self.nc, 1), 1)
        # x_cat = [bbox | cls | visibility]
        box, cls, _ = x_cat.split((self.reg_max * 4, self.nc, 1), 1)
